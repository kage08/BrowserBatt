...
def run_workload(
    config: BrowserBattConfig, workload: str, out_root: Path, dry_run: bool = False
) -> Path:
    ...
    try:
        with prevent_sleep(f"BrowserBatt {workload}") as caffeinate:
            ...
            for browser in browsers:
                ...
                _progress(
                    f"Cooldown after warmup {browser}: {measurement.cooldown_seconds}s"
                )
            ...
            for rep_index, order in enumerate(orders, start=1):
                for browser in order:
                    _preflight(
                        config, dry_run=dry_run, context=f"rep {rep_index} {browser}"
                    )
                    duration = int(
                        config.workloads[workload].get("duration_seconds", 720)
                    )
                    _progress(
                        f"Measured run {rep_index}/{measurement.repetitions} {browser}: {duration}s"
                    )
                    _one_run(
                        root,
                        config,
                    ...
                    _progress(
                        f"Cooldown after rep {rep_index} {browser}: {measurement.cooldown_seconds}s"
                    )
                    _cooldown(measurement.cooldown_seconds)
        ...
        append_jsonl(
            root / "events.jsonl",
            {"timestamp": time.time(), "event": "run_error", "error": repr(exc)},
        )
        raise
    finally:
        try:
...
def _one_run(
    ...
    try:
        ...
        sampler = Sampler(
            ...
            config.measurement.use_powermetrics,
        )
        sampler.start()
        ...
        if duration_seconds > 0:
            runner = WorkloadRunner(
                ...
                config.automation.typing_cadence_seconds,
            )
        ...
        append_jsonl(
            run_dir / "events.jsonl",
            {"timestamp": time.time(), "event": "run_error", "error": error},
        )
        raise
    finally:
...
def _baseline(
    out_dir: Path, seconds: int, measurement, config: BrowserBattConfig, dry_run: bool
) -> None:
    _preflight(config, dry_run=dry_run, context=f"baseline {out_dir.name}")
    sampler = Sampler(
        out_dir, None, measurement.sample_interval_seconds, measurement.use_powermetrics
    )
    sampler.start()
    try:
...
def _preflight(config: BrowserBattConfig, dry_run: bool, context: str) -> None:
    ...
    if config.measurement.require_battery_power and not dry_run:
        if status.power_source and "battery" not in status.power_source.lower():
            raise RuntimeError(
                f"Mac is not on battery power during {context}: {status.power_source}"
            )
        if status.is_charging:
            raise RuntimeError(f"Mac appears to be charging during {context}")
    if (
        status.percent is not None
        and status.percent < config.measurement.min_battery_percent
    ):
        raise LowBatteryStop(
            f"Battery is below minimum threshold during {context}: {status.percent}%"
        )
...
def _write_cleanup_notes(root: Path) -> None:
    ...
    for events in root.glob("*/**/events.jsonl"):
        for line in events.read_text(encoding="utf-8").splitlines():
            ...
            if (
                url
                and url.startswith("https://docs.google.com/document/d/")
                and url not in urls
            ):
                urls.append(url)
    if not urls:
